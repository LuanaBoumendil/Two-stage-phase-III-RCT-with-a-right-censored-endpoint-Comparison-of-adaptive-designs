...
Handicap0.5_N86_d66_K1 <- c(0.101, 0.094, 0.094, 0.093, 0.085, 0.085, 0.09, 0.083, 0.083, 
                            0.082, 0.085, 0.049, 0.048, 0, 0, 0, 0)
Handicap0.67_N234_d192_K1 <- c(0.095, 0.099, 0.107, 0.116, 0.119, 0.125, 0.127, 0.127, 0.117, 
                               0.117, 0.113, 0.095, 0.082, 0.073, 0.041, 0, 0)
Handicap0.8_N748_d633_K1 <- c(0.1, 0.109, 0.118, 0.127, 0.129, 0.131, 0.133, 0.133, 0.127, 
                              0.12, 0.127, 0.118, 0.098, 0.093, 0.078, 0.04, 0)
...
beta <- 0.2
alpha <- 0.05
K = 1 # shape de la weibull modelisant les temps d'evenements dans le groupe experimental
#lambda1 <- log(2)^(1/K)/12
...
Get_Sample_Size_for_TAU0.5 <- function(alpha,beta,lambda0,lambda1,end_study_after_nb_month, end_accrual_after_nb_month){
  tau <- 0.5
...
Get_Analysis_Time <- function(TAU,alpha,beta,lambda0,lambda1,end_study_after_nb_month, end_accrual_after_nb_month){
  ...
  for(b in 1:length(TAU)){
    ...
    AI[b] <- DesignPlan$analysisTime[1] 
  }
  TAU <- c(TAU,1)
  AI <- c(AI,60)
  return(data.frame(TAU,AI))
}

AI <- as.numeric(Get_Analysis_Time(TAU=c(seq(0.2,0.95,0.05)) ,alpha,beta,lambda0,lambda1,end_study_after_nb_month=60, end_accrual_after_nb_month=40)$AI)
...
Get_reject_proportion <- function(nb_simu,N,d,lambda0,lambda1,K,K0, ta, end_study,TAU,AI,vect_handicap){
  var = 4 # approximation 
  early_stop <- c()
  power <- c()
  for(b in 1:length(TAU)){
    ...
    for(i in 1:nb_simu){
      ...
      if(tau!=1&d1<d){
        fit1 <- coxph(Surv(tt,evt)~trait,data=filter(data,recrutement<tps_interim) %>% mutate(evt=ifelse(time_since_study_begening>=tps_interim,0,evt)) %>% mutate(
          tt=ifelse(time_since_study_begening>=tps_interim,tps_interim-recrutement,tt)))
        ...
        fit2 <- coxph(Surv(tt,evt)~trait,data=data)
        Zstat2 <- summary(fit2)$coef[1]
        decision[i] = ifelse(abs(Zstat2)> qnorm(1-alpha/2)/sqrt(d/var),1,0)#handicap=0
        efficacy[i] = 0
      }
...
Get_reject_proportion(nb_simu=10,N,d,lambda0,lambda1,K=1,K0=1, ta, end_study,TAU=TAU,AI=AI,vect_handicap=Handicap0.5_N86_d66_K1 )
